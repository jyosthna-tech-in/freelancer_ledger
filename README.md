...
| Path | Role |
| --- | --- |
| `frontend/` | React + Vite + TypeScript UI (not scaffolded yet) |
| `backend/` | AWS SAM + TypeScript Lambda handlers, tests, events, env (not scaffolded yet) |
| `docs/` | Scope, architecture, and patch order |
| `scripts/` | Local helper scripts (later) |
| `infra/` | Infrastructure templates (later) |

## Current status

**Patch 1 — repository foundation.** This repository contains documentation, ignore rules, environment placeholders, and empty package directories. There is no executable application logic yet. Follow [`docs/PATCH_PLAN.md`](docs/PATCH_PLAN.md) for the implementation order from setup through local testing.
